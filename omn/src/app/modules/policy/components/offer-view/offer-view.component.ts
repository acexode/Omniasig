...
export class OfferViewComponent implements OnInit {
  ...
  openIAB(url, type) {
    const options = 'location=no,hidden=yes,footer=no,hardwareback=no,hidenavigationbuttons=yes,clearcache=yes,clearsessioncache=yes,toolbar=no';
    ...
    if (browser) {
      this.sub = browser.on('loadstart').subscribe((e) => {
        browser.insertCSS({ code: '.header__cancel{ display: none;}' }).then((res) => console.log(res)).catch((err) => console.log(err));
